...
func (conR *Reactor) Start() error {
	return conR.OnStart()
}

func (conR *Reactor) Stop() error {
	conR.OnStop()
	return nil
}
...
func (conR *Reactor) OnStart() error {
	log.Info("Reactor ", "waitSync", conR.WaitSync())
...
func (conR *Reactor) SwitchToConsensus(state sm.State, _ bool) {
...
func (conR *Reactor) Receive(chID byte, src p2p.Peer, msgBytes []byte) {
	...
	if err != nil {
		log.Error("Error decoding message", "src", src, "chId", chID, "err", err)
		conR.Switch.StopPeerForError(src, err)
		return
	}

	if err = msg.ValidateBasic(); err != nil {
		log.Error("Peer sent us invalid msg", "peer", src, "msg", msg, "err", err)
		conR.Switch.StopPeerForError(src, err)
		return
	}
	...
	case DataChannel:
		if conR.WaitSync() {
			log.Info("Ignoring message received during sync", "msg", msg)
			return
		}
		switch msg := msg.(type) {
...
func (conR *Reactor) subscribeToBroadcastEvents() {
	...
	if err := conR.conS.evsw.AddListenerForEvent(subscriber, types.EventNewRoundStep,
		...
		log.Error("Error adding listener for events", "err", err)
	}
...
OUTER_LOOP:
	for {
		...
		blockStoreBase := conR.conS.commitStore.Base()
		if blockStoreBase > 0 && 0 < prs.Height && prs.Height < rs.Height && prs.Height >= blockStoreBase {
			// if we never received the commit message from the peer, the block parts wont be initialized
			if prs.ProposalBlockParts == nil {
				blockMeta := conR.conS.commitStore.LoadBlockMeta(prs.Height)
				if blockMeta == nil {
					log.Error("height: Failed to load block meta",
						"blockstoreBase", blockStoreBase, "blockstoreHeight", conR.conS.commitStore.Height())
		...
		if (rs.Height != prs.Height) || (rs.Round != prs.Round) {
			// log.Info("Peer Height|Round mismatch, sleeping",
...
func (conR *Reactor) gossipDataForCatchup(rs *cstypes.RoundState,
	...
	if index, ok := prs.ProposalBlockParts.Not().PickRandom(); ok {
		...
		blockMeta := conR.conS.commitStore.LoadBlockMeta(prs.Height)
		if blockMeta == nil {
			log.Error("Failed to load block meta", "ourHeight", rs.Height,
				"blockstoreBase", conR.conS.commitStore.Base(), "blockstoreHeight", conR.conS.commitStore.Height())
			time.Sleep(conR.conS.config.PeerGossipSleepDuration)
			return
		} else if !blockMeta.BlockID.PartSetHeader.Equals(prs.ProposalBlockPartSetHeader) {
			log.Info("Peer ProposalBlockPartSetHeader mismatch, sleeping",
				"blockPartSetHeader", blockMeta.BlockID.PartSetHeader, "peerBlockPartSetHeader", prs.ProposalBlockPartSetHeader)
			time.Sleep(conR.conS.config.PeerGossipSleepDuration)
			return
		}
		// Load the part
		part := conR.conS.commitStore.LoadBlockPart(prs.Height, index)
		if part == nil {
...
OUTER_LOOP:
	for {
		...
		if blockStoreBase > 0 && prs.Height != 0 && rs.Height >= prs.Height+2 && prs.Height >= blockStoreBase {
			...
			if commit := conR.conS.commitStore.LoadBlockCommit(prs.Height); commit != nil {
...
OUTER_LOOP:
	for {
		...
		// TODO: Disabled to remove responsibility of State to serve commits
		...
		{
			prs := ps.GetRoundState()
			if prs.CatchupCommitRound != -1 && prs.Height > 0 && prs.Height <= conR.conS.commitStore.Height() &&
				prs.Height >= conR.conS.commitStore.Base() {
...
func (conR *Reactor) peerStatsRoutine() {
	for {
		if !conR.IsRunning() {
			log.Info("Stopping peerStatsRoutine")
			return
		}
		...
		case msg := <-conR.conS.statsMsgQueue:
			...
			if peer == nil {
				log.Debug("Attempt to update stats for non-existent peer",
					"peer", msg.PeerID)
				continue
			}
			// Get peer state
...
func (ps *PeerState) setHasVote(height int64, round int32, voteType tmproto.SignedMsgType, index int32) {
	log.Debug("setHasVote",
		"peerH/R",
		lazyfmt.NewLazySprintf("%d/%d", ps.PRS.Height, ps.PRS.Round),
		"H/R",
		lazyfmt.NewLazySprintf("%d/%d", height, round),
