...
import (
	...
	"github.com/Wondertan/iotwal/lazyfmt"
...
func (cs *State) LoadCommit(height int64) *types.Commit {
	...
	if height == cs.commitStore.Height() {
		return cs.commitStore.LoadSeenCommit(height)
	}

	return cs.commitStore.LoadBlockCommit(height)
}
...
func (cs *State) OnStop() {
	if err := cs.evsw.Stop(); err != nil {
		log.Error("failed trying to stop eventSwitch", "error", err)
	}
...
func (cs *State) reconstructLastCommit(state sm.State) {
	seenCommit := cs.commitStore.LoadSeenCommit(state.LastBlockHeight)
	if seenCommit == nil {
		panic(fmt.Sprintf(
...
func (cs *State) newStep() {
	...
	if cs.eventBus != nil {
		if err := cs.eventBus.PublishEventNewRoundStep(rs); err != nil {
			log.Error("failed publishing new round step", "err", err)
		}
...
func (cs *State) handleTimeout(ti timeoutInfo, rs cstypes.RoundState) {
	...
	case cstypes.RoundStepPropose:
		if err := cs.eventBus.PublishEventTimeoutPropose(cs.RoundStateEvent()); err != nil {
			log.Error("failed publishing timeout propose", "err", err)
		}
	...
	case cstypes.RoundStepPrevoteWait:
		if err := cs.eventBus.PublishEventTimeoutWait(cs.RoundStateEvent()); err != nil {
			log.Error("failed publishing timeout wait", "err", err)
		}
...
func (cs *State)  enterNewRound(height int64, round int32) {
	...
	if cs.Height != height || round < cs.Round || (cs.Round == round && cs.Step != cstypes.RoundStepNewHeight) {
		logger.Debug(
			...
			"current", lazyfmt.NewLazySprintf("%v/%v/%v", cs.Height, cs.Round, cs.Step),
		)
		return
	}
	...
	logger.Debug("entering new round", "current", lazyfmt.NewLazySprintf("%v/%v/%v", cs.Height, cs.Round, cs.Step))
	...
	if err := cs.eventBus.PublishEventNewRound(cs.NewRoundEvent()); err != nil {
		log.Error("failed publishing new round", "err", err)
	}
...
func (cs *State) needProofBlock(height int64) bool {
	...
	lastBlockMeta := cs.commitStore.LoadBlockMeta(height - 1)
...
func (cs *State) enterPropose(height int64, round int32) {
	...
	logger.Debug("entering propose step", "current", lazyfmt.NewLazySprintf("%v/%v/%v", cs.Height, cs.Round, cs.Step))
...
func (cs *State) enterPrevote(height int64, round int32) {
	...
	logger.Debug("entering prevote step", "current", lazyfmt.NewLazySprintf("%v/%v/%v", cs.Height, cs.Round, cs.Step))
...
func (cs *State) enterPrevoteWait(height int64, round int32) {
	...
	logger.Debug("entering prevote wait step", "current", lazyfmt.NewLazySprintf("%v/%v/%v", cs.Height, cs.Round, cs.Step))
...
func (cs *State) enterPrecommit(height int64, round int32) {
	...
	logger.Debug("entering precommit step", "current", lazyfmt.NewLazySprintf("%v/%v/%v", cs.Height, cs.Round, cs.Step))
...
func (cs *State) enterPrecommitWait(height int64, round int32) {
	...
	if cs.Height != height || round < cs.Round || (cs.Round == round && cs.TriggeredTimeoutPrecommit) {
		logger.Debug(
			...
			"current", lazyfmt.NewLazySprintf("%v/%v", cs.Height, cs.Round),
		)
		return
	}
	...
	logger.Debug("entering precommit wait step", "current", lazyfmt.NewLazySprintf("%v/%v/%v", cs.Height, cs.Round, cs.Step))
...
func (cs *State) enterCommit(height int64, commitRound int32) {
	logger := log.With("height", height, "commit_round", commitRound)
	...
	logger.Debug("entering commit step", "current", lazyfmt.NewLazySprintf("%v/%v/%v", cs.Height, cs.Round, cs.Step))
	...
	if !cs.ProposalBlock.HashesTo(blockID.Hash) {
		if !cs.ProposalBlockParts.HasHeader(blockID.PartSetHeader) {
			logger.Info(
				...
				"proposal", lazyfmt.NewLazyBlockHash(cs.ProposalBlock),
				"commit", blockID.Hash,
			)
...
func (cs *State) tryFinalizeCommit(height int64) {
	...
	if !cs.ProposalBlock.HashesTo(blockID.Hash) {
		...
		logger.Debug(
			...
			"proposal_block", lazyfmt.NewLazyBlockHash(cs.ProposalBlock),
...
func (cs *State) finalizeCommit(height int64) {
	...
	logger.Info(
		...
		"hash", lazyfmt.NewLazyBlockHash(block),
		"root", block.AppHash,
		"num_txs", len(block.Txs),
	)
	logger.Debug("committed block", "block", lazyfmt.NewLazySprintf("%v", block))

	fail.Fail() // XXX

	// TODO: Block storing was removed
...
func (cs *State) addProposalBlockPart(msg *BlockPartMessage, peerID p2p.ID) (added bool, err error) {
	...
	if added && cs.ProposalBlockParts.IsComplete() {
		...
		if err := cs.eventBus.PublishEventCompleteProposal(cs.CompleteProposalEvent()); err != nil {
			log.Error("failed publishing event complete proposal", "err", err)
		}
	}
	return added, nil
}
	...
	if hasTwoThirds && !blockID.IsZero() && (cs.ValidRound < cs.Round) {
		if cs.ProposalBlock.HashesTo(blockID.Hash) {
			log.Debug(
				...
				"valid_block_hash", lazyfmt.NewLazyBlockHash(cs.ProposalBlock),
			)
...
func (cs *State) addVote(vote *types.Vote, peerID p2p.ID) (added bool, err error) {
	...
	if vote.Height+1 == cs.Height && vote.Type == tmproto.PrecommitType {
		...
		log.Debug("added vote to last precommits", "last_commit", cs.LastCommit.StringShort())
...
func (cs *State) checkDoubleSigningRisk(height int64) error {
	if cs.privValidator != nil && cs.privValidatorPubKey != nil && cs.config.DoubleSignCheckHeight > 0 && height > 0 {
		...
		for i := int64(1); i < doubleSignCheckHeight; i++ {
			lastCommit := cs.commitStore.LoadSeenCommit(height - i)
			if lastCommit != nil {
