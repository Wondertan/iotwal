...
func TestByzantinePrevoteEquivocation(t *testing.T) {
	...
	for i := 0; i < nValidators; i++ {
		...
		case cfg.MempoolV1:
			mempool = mempoolv1.NewTxMempool(consensusLogger(),
				config.Mempool,
				proxyAppConnConMem,
				state.LastBlockHeight,
		...
		// Make State
		blockExec := sm.NewBlockExecutor(stateStore, consensusLogger(), proxyAppConnCon, mempool, evpool)
	...
	lazyProposer.decideProposal = func(height int64, round int32) {
		...
		default: // This shouldn't happen.
			t.Error("enterPropose: Cannot propose anything: No commit for the previous block")
			return
		}
		...
		if lazyProposer.privValidatorPubKey == nil {
			...
			t.Error(fmt.Sprintf("enterPropose: %v", errPubKeyIsNotSet))
			return
		}
		...
		if err := lazyProposer.privValidator.SignProposal(lazyProposer.state.ChainID, p); err == nil {
			...
			t.Log("Signed proposal", "height", height, "round", round, "proposal", proposal)
			t.Log(fmt.Sprintf("Signed proposal block: %v", block))
		} else {
			t.Error("enterPropose: Error signing proposal", "height", height, "round", round, "err", err)
		}
	}
	...
	defer stopConsensusNet(t, reactors, eventBuses)
